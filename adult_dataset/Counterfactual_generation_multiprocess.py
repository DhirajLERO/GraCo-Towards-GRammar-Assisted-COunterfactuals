...
def process_data(df, model, scaler, columns_to_standardize):
    ...
    input_0 = df_0.iloc[:, 0:13].values
    input_1 = df_1.iloc[:, 0:13].values
...
def create_input_data(data_file_loc, grammar_loc, model, bounds, scaler, columns_to_standardize ):
    ...
    # print(df.columns)
    df = filter_dataframe(df, bounds, drop_columns=["class"])
...
if __name__ == "__main__":
    ...
    multiobj = True
    algo = "NSGAIII"
    save_file_location = "output/" + algo + "_multi_200/"
