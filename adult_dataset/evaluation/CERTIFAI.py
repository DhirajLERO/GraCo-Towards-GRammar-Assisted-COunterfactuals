...
class CERTIFAI:
    ...
    def generate_prediction(self, model, model_input, pytorch=True, classification = True, experiment=False):
        ...
        if classification:
            ...
            prediction = []
            with torch.no_grad():
                probability = model(model_input).tolist()
                # print(probability)
    ...
    def fit(self, 
        ...
        if self.predictions is None: 
            self.predictions = self.generate_prediction(model, model_input,
                                                  pytorch = pytorch,
                                                  classification = classification,experiment=experiment)
        ...
        for i in tot_samples:
            ...
            for g in range(generations):
                ...
                # print("predictions: ", counter_preds)
                # print("self.predictions[g]: ", self.predictions[g])

                diff_prediction = [counter_pred!=self.predictions[i] for
                                   counter_pred in counter_preds]

                # print("diff_pred: ", diff_prediction)

                final_generation = crossed_generation.loc[diff_prediction]

                # print(sample.shape, final_generation.shape)
