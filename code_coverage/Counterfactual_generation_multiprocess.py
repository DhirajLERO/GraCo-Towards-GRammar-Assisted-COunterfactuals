...
def process_data(df, model):
    with torch.no_grad():
        y_pred = model(torch.tensor(df.iloc[:, 0:9].values, dtype=torch.float32))
    ...
    input_0 = df_0.iloc[:, 0:9].values
    input_1 = df_1.iloc[:, 0:9].values
...
def create_input_data(data_file_loc, grammar_loc, model, bounds ):
    ...
    # print(df.columns)
    df = filter_dataframe(df, bounds, drop_columns = ["class", 'Unnamed: 0'])
    ...
    if individual.invalid == True:
        ...
        try:
            ...
            if not check_bounds(x, bounds):
                # print("Individual: ", x , ", out of bound")
                if multiobj:
                    individual.o_1 = 10000
                    individual.o_2 = 10000
                    individual.o_3 = 10000
                    individual.o_4 = 10000
                    # return np.NaN, np.NaN, np.NaN, np.NaN
...
def generate_counterfactuals_wrapper(index, save_file_location, input_0, algo, fitness_eval_MOC, multiobj, bounds, ref_points, N_RUNS, run, MU, BNF_GRAMMAR, MIN_INIT_TREE_DEPTH, MAX_INIT_TREE_DEPTH, CODON_SIZE, CODON_CONSUMPTION, GENOME_REPRESENTATION, HALLOFFAME_SIZE, P_CROSSOVER, P_MUTATION, MAX_GENERATIONS, ELITE_SIZE, MAX_TREE_DEPTH, MAX_GENOME_LENGTH, REPORT_ITEMS, problem):
...
if __name__ == "__main__":
    ...
    multiobj = True
    algo = "NSGAIII"
    save_file_location = "output/" + algo + "test_multi/"
    ...
    model = load_model()
    input_0, input_1, BNF_GRAMMAR = create_input_data("test.csv", "./../grammars/multiple_type.bnf", model, bounds)
